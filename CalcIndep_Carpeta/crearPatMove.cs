...
namespace CalcIndep_Carpeta
{
    public static class crearPatMove
    {
        ...
        public static bool enviarCorrimientos(Patient paciente, VVector iso, int i, PlanSetup plan)
        {
            ...
            List<object> textoAEnviar = new List<object> { paciente.Id, paciente.LastName.ToUpper() + ", " + paciente.FirstName + "_" + plan.Id, "Iso " + (i + 1).ToString(), desplazX.ToString(), sentidoX, desplazY.ToString(), sentidoY, desplazZ.ToString(), sentidoZ, DateTime.Today.ToString("dd-MM-yyyy",CultureInfo.InvariantCulture) };
