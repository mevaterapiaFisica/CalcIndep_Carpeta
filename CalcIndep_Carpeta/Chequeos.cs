    ...
    public static class Chequeos
    {
        ...
        public static string hayTratamientoAprobado(PlanSetup planActual)
        {
            ...
            foreach (Course curso in paciente.Courses)
            {
                ...
                {
                    if (plan.Id != planActual.Id && plan.Course.Id != planActual.Course.Id)
                    {
        ...
        public static string advertenciaTiempoVMAT(PlanSetup plan)
        {
            string texto = "";
            if (plan.Beams.First().MLCPlanType==MLCPlanType.VMAT)
            {
                texto += "\nPlan VMAT: Recordar poner tiempos por arco de al menos 2 minutos";
            }
            return texto;
        }
        ...
        public static string planNoAprobado(PlanSetup plan)
        {
            ...
            if (plan.Id.Contains("CI"))
            {
                ...
                {
                    if (planAp.ApprovalStatus == PlanSetupApprovalStatus.PlanningApproved && planAp.ApprovalStatus == PlanSetupApprovalStatus.TreatmentApproved)
                    {
                        return texto;
                    }
                }
        ...
        public static string tieneCamposSetUp(PlanSetup plan)
        {
            string texto = "";
            bool hayCampoDeSetUp = false;
            foreach (Beam campo in plan.Beams)
            {
                if (campo.IsSetupField)
                {
                    ...
                    {
                        texto += "\nEl campo " + campo.Id + " no tiene imágen DRR";

                    }
                    hayCampoDeSetUp = true;
                }
            }
            if (!hayCampoDeSetUp)
            {
                texto += "\nEl plan no tiene campos de Set Up";
            }
            return texto;
        }

        /*public static string tablaTolerancia(Beam campo)
        {
            string texto = "";
            if (campo.ToleranceTableLabel =="")
            {
                texto += "\n" + campo.Id + ": el campo no tiene tabla de tolerancia";
            }
            else if 
        }
        ...
        public static string chequeos(PlanningItem plan, bool esPlanSuma)
        {
            ...
            if (!esPlanSuma)
            {
                ...
                texto += planNoAprobado((PlanSetup)plan);
                texto += hayTratamientoAprobado((PlanSetup)plan);
                texto += advertenciaTiempoVMAT((PlanSetup)plan);
                texto += orientacionArcos((PlanSetup)plan);
                //texto += tieneCamposSetUp((PlanSetup)plan);
            }
            else
            {
                ...
                foreach (PlanSetup etapa in ((PlanSum)plan).PlanSetups)
                {
                    ...
                    aux += planNoAprobado(etapa);
                    aux += advertenciaTiempoVMAT(etapa);
                    aux += orientacionArcos((PlanSetup)etapa);
                    //aux += tieneCamposSetUp(etapa);
                    if (aux != "")
                    {
                        texto += "\n" + etapa.Id + ": " + aux + "\n";
                    }
...
- ¿Falta hacer merge de campos? LISTO
    ...
    FALTA (06-09-19)
    - orden de campo (Rafa)
    - El plan está aprobado (para chequeo de armado) LISTO
